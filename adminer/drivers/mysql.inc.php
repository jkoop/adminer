...
if (!defined("DRIVER")) {
	...
	*/
	function check_constraint($name, $table) {
		...
		$rows = get_rows("SELECT `CHECK_CLAUSE` FROM `information_schema`.`CHECK_CONSTRAINTS` WHERE `CONSTRAINT_SCHEMA` = " . q(DB) . " AND `TABLE_NAME` = " . q($table) . " AND `CONSTRAINT_NAME` = " . q($name));
	...
	*/
	function check_constraints($table) {
		$return = array();
		foreach (get_rows("SELECT `CONSTRAINT_NAME`, `CHECK_CLAUSE` FROM `information_schema`.`CHECK_CONSTRAINTS` WHERE `CONSTRAINT_SCHEMA` = " . q(DB) . " AND `TABLE_NAME` = " . q($table)) as $row) {
